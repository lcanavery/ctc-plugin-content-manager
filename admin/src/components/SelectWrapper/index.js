...
const history =useHistory();
  ...
  const { pathname,...itris } = useLocation();
  ...
  let toOnePagerEn = `/empresas/productos/${initialData.id?initialData.id:null}/onepager/${
    value?.nombre ? value?.nombre : "default"
  }/EN/pdf`;
  let toOnePagerEs = `/empresas/productos/${initialData.id?initialData.id:null}/onepager/${
    value?.nombre ? value?.nombre : "default"
  }/ES/pdf`;
  ...
  const link = useMemo(() => {
    ...
    if(!initialData.id && name === "onepager" )
      return null
...
<a target="_blank" href={`${window.location.origin+toOnePagerEs}`} style={{paddingRight:'10px'}}>
                      Español
                      </a>
                      <a  target="_blank" href={`${window.location.origin+toOnePagerEn}`}>
                     Ingles
                      </a>
</div>
:
